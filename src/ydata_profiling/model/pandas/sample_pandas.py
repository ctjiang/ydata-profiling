...
def pandas_get_sample(config: Settings, df: pd.DataFrame) -> List[Sample]:
    ...
    if n_head > 0:
        samples.append(Sample(id="head", data=df.head(n=n_head), name=_("First rows")))

    n_tail = config.samples.tail
    if n_tail > 0:
        samples.append(Sample(id="tail", data=df.tail(n=n_tail), name=_("Last rows")))

    n_random = config.samples.random
    if n_random > 0:
        samples.append(
            Sample(id="random", data=df.sample(n=n_random), name=_("Random sample"))
        )
