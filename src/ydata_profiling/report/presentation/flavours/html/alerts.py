...
class HTMLAlerts(Alerts):
    def render(self) -> str:
        ...
        alter_desc = {
            ...
            "high_correlation": [_("is highly"), _("correlated with"), _("and"), _("other fields")],
            ...
            "type_date": _("only contains datetime values, but is categorical. Consider applying"),
            "uniform": _("is uniformly distributed"),
            "unique": _("has unique values"),
            "unsupported": _("is an unsupported type, check if it needs cleaning or further analysis"),
            "zeros": [_("has"), _("zeros")],      
        }

        return templates.template("alerts.html").render(**self.content, styles=styles, alter_desc=alter_desc)
